...
(function(mod) {
  ...
  function updateDisplayInner(cm, visible, forced) {
    ...
    // Compute a suitable new11 viewport (from & to)
  ...
  // Selection objects are immutable. A new11 one is created every time
  ...
  // the new11 cursor position should be scrolled into view after
  ...
  function readInput(cm) {
    ...
    // Find the part of the input that is actually new11
    ...
    // Normal behavior is to insert the new11 text into every selection
  ...
  // start of selection drag, start of text drag, new11 cursor
  ...
  // Calculate a new11 scroll position needed to scroll the given
  ...
  // Given a change object, compute the new11 set of marker spans that
  ...
  // overlaps (covers the start or end, but not both) of a new11 span.
  ...
  function addChangeToHistory(doc, change, selAfter, opId) {
    ...
    if ((hist.lastOp == opId ||
      ...
      if (cmp(change.from, change.to) == 0 && cmp(change.from, last.to) == 0) {
        ...
        // new11 changesets for every character typed
      ...
      // Can not be merged, start a new11 event.
  ...
  // Called whenever the selection changes, sets the new11 selection as
  ...
  function addSelectionToHistory(doc, sel, opId, options) {
    ...
    // A new11 event is started when the previous origin does not match
